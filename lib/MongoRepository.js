...
module.exports.create = function(mongoose, models) {
	...
	// Messaggi di errore
	var ERROR_NO_EVENT = 'nessun evento specificato';
	...
	var list = function(options, callback) {
		...
		if (author) {			
			...
			// Ricerca per Tag, per Autore e OmniSearch. Infine, unisce i risultati
			searchByTagOnOmniSearch(conditions, search, maxlen, maxLimit)
				.then(function(docs) {
					return searchByAuthorOnOmniSearch(conditions, search, maxlen, maxLimit, docs);
				})
				.then(function(docs) {
					return searchByWordsOnOmniSearchAnd(conditions, search, maxlen, maxLimit, docs);
				})
				.then(function(docs) {
					if (0 === docs.length) {
						searchByWordsOnOmniSearchOr(conditions, search, maxlen, maxLimit).then(function(docs) {
							callback(false, docs);
						});
					} else {
						callback(false, docs);
	 ...
	 */
	var logEvent = function(options, callback) {
		var deviceUUID = options.deviceUUID || 'testclient',
			userProfile = options.userProfile || systemProfile,
			data = options.data || {};
		
		if (!options.event) {
			callback(true, ERROR_NO_EVENT);	
		} else {
			saveHistory(userProfile.id, deviceUUID, options.event.toLowerCase(), data);
			callback(false, deviceUUID);	
		}		
	};
	
	var searchByTagOnOmniSearch = function(conditions, search, maxlen, maxLimit) {
		initConditions(conditions, maxlen);
		addCondition(conditions, 'tags.name', eval('/' + search + '/i'), '$and');
						
		return searchByConditions(conditions, maxlen, maxLimit);
	};
	
	var searchByAuthorOnOmniSearch = function(conditions, search, maxlen, maxLimit, docs) {
		initConditions(conditions, maxlen);
		addCondition(conditions, 'author', eval('/' + search + '/i'), '$and');
		
		return searchByConditions(conditions, maxlen, maxLimit, docs);
	};
	
	var searchByWordsOnOmniSearchAnd = function(conditions, search, maxlen, maxLimit, docs) {
		initConditions(conditions, maxlen);
	...
	var searchByWordsOnOmniSearchOr = function(conditions, search, maxlen, maxLimit) {
	...
	var searchByConditions = function(conditions, maxlen, maxLimit, oldDocs) {
		...
		countElements(models.Quote, conditions).then(function(result) {	
			if (0 !== result) {	
				findRandomElements(conditions, result, maxLimit).then(function(docs) {
					toReturn = mergeDocs(toReturn, docs, maxLimit);
	...
	var mergeDocs = function(firstBlock, secondBlock, limit) {
		var stop = firstBlock.length >= limit,
			i = 0,
			toReturn = firstBlock,
			foundQuote;
		
		if (firstBlock.length === 0) {
			toReturn = secondBlock;
		} else {			
			while (!stop && i < secondBlock.length) {
				foundQuote = _.find(firstBlock, function(quote) {
					return (quote.text === secondBlock[i].text);
				});
				if (!foundQuote && toReturn.length < limit) {
					toReturn.push(secondBlock[i]);
				}
				if (toReturn.length === limit || i === secondBlock.length) {
					stop = true;
				} else {
					i++;
				}
	...
	var findRandomElements = function(conditions, count, maxLimit) {
		...
		models.Quote.findRandom(conditions, 'author text tags', getFindOptions(count, maxLimit), function (err, docs) {
			if (err) {
				deferred.reject(err);
			} else {
	 ...
	 */
	var saveHistory = function(userProfileId, deviceUUID, eventType, data) {
		var newData = data || {};
		delete newData.event;
		delete newData.deviceUUID;
