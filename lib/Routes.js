...
module.exports.create = function(mongoose, models) {
	...
	var removeWrongQuotes = function(request, response) {
		...
		models.Quote.find({ author: 'Autore di prova' }, function(err, docs) {
			if (!err) {
				console.log("Numero di citazioni: " + docs.length);
				
				docs.forEach(function(elem, index, array) {	
					functions.push(checkWrongQuote(elem, function(callbackError, callbackResult) {
						if (callbackResult.toRemove) {
							callbackResult.quote.remove();
						}
						console.log("Callback Result: " + callbackResult.toRemove);
					}));
				});			
				
				async.series(functions, function(err, results) {
					results.forEach(function(result) {
						count = count + (true === result.toRemove ? 1 : 0);
					});
					console.log("Citazioni errate: " + count);
	...
	var checkWrongQuote = function(quote, callback) {
		var toRemove;
