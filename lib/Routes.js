	...
	var removeWrongQuotes = function(request, response) {				
		...
		models.Quote.find({}, function(err, docs) {
			if (!err) {
				...
				// Spostare foreach dentro la promise
				// Al then fare handleResponse
				docs.forEach(function(elem, index, array) {
							
					// Conta citazioni
					countElements(models.Quote, { 'text': elem.text }).then(function(result) {
						toRemove = false;
						...
						// Autore con iniziali non alfanumeriche 
						if(elem.author.search(/^[A-ZÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÑÒÓÔÕÖØÙÚÛÜÝßĀĂĄĆĈĊČĎĒĔĖĘĚĜĞĢĤ]/) !== 0) {						
							toRemove = true;
						}					
