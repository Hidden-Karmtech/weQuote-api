...
module.exports.create = function(mongoose, models) {
	...
	var removeQuotesWithTextDuplicated = function(request, response) {				
		// TODO
	};
	
	var removeQuotesWithWrongAuthor = function(request, response) {				
		var count = 0;
		models.Quote.find({}, function(err, docs) {
			if (!err) {
				docs.forEach(function(elem, index, array) {
					if(elem.author.search(/^[A-Z]/) != 0) {
						count++;
						//elem.remove();
					}					
				});				
	...
	var removeQuotesWithShortText = function(request, response) {				
		var count = 0;
		models.Quote.find({}, function(err, docs) {
			if (!err) {
				docs.forEach(function(elem, index, array) {
					if (elem.text.length < MIN_LEN) {
						count++;
						elem.remove();
					}					
				});				
	...
	var quoteExists = function(request, response) {
		...
		// Imposta condizione
		conditions.$and = [];
		addCondition(conditions, 'text', search, '$and');
	...
	var share = function(request, response) {
		var quoteId = request.query.quoteId,
			deviceUUID = request.query.deviceUUID || 'testclient',
			userProfile = request.query.userProfile || systemProfile;
	...
	return {
		...
		removeQuotesWithShortText: removeQuotesWithShortText,
		removeQuotesWithTextDuplicated: removeQuotesWithTextDuplicated,
		removeQuotesWithWrongAuthor: removeQuotesWithWrongAuthor,
		share: share,		
		tags: tags
	};
};