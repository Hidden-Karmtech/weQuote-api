...
module.exports.create = function(mongoRepository) {
	...
	var authors = function(request, response) {		
		mongoRepository.authors(function(err, result) {
			handleResponse(response, err, _.sortBy(adaptResults(result, 'name'), function(author) {
				return author;
			}));
		});
	};
	...
	var tags = function(request, response) {		
		mongoRepository.tags(function(err, result) {
			handleResponse(response, err, _.sortBy(adaptResults(result, 'name'),function(tag) {
				return tag.name;
			}));
		});
	};
	...
	var handleResponse = function(response, err, result) {
		...
		response.writeHead(returnCode, { "Content-Type": "application/json" });
